        ...
        <div class="container">
          <div class="row mt-4">
            <div class="col-lg-12 mb-lg-4 mb-4">
              <div class="card z-index-2 h-100 mx-auto p-4" id="prediction-card" style="box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                <div class="card-header pb-0 bg-transparent">
                  <h6 class="text-center text-capitalize"><b>Prediksi Kualitas Udara</b></h6>
                </div>
                <div class="card-body p-3">
                  <div class="row text-center">
                    <!-- AQ1 Prediction -->
                    <div class="col-lg-6 mb-4">
                      <div class="p-3 rounded shadow-sm" id="aqiCard">
                        <i class="fa fa-smog fa-2x mb-2 text-primary"></i>
                        <h5 class="font-weight-bold">Prediksi AQ</h5>
                        <h6 class="mb-0" id="prediksiAQ1">CO2: Loading...</h6>
                        <h6 class="mb-0" id="qualityAQ1">Kualitas: Loading...</h6>
                      </div>
                    </div>

                    <!-- CEMS Prediction Card -->
                    <div class="col-lg-6 mb-4">
                      <div class="p-3 rounded shadow-sm" id="cemsCard">
                        <i class="fa fa-cloud fa-2x mb-2 text-primary"></i>
                        <h5 class="font-weight-bold">Prediksi CEMS</h5>
                        <h6 class="mb-0" id="prediksiCEMS1">CO2: Loading...</h6>
                        <h6 class="mb-0" id="qualityCEMS1">Kualitas: Loading...</h6>
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    function fetchData2() {
                      $.ajax({
                        ...
                        success: function(data) {
                          ...
                          // Anggap data sudah diambil dan predik sudah terisi
                          const predik = data.predik[0];

                          // Menampilkan prediksi CO2
                          $('#prediksiAQ1').text(predik.coaq);
                          $('#prediksiCEMS1').text(predik.cocems);

                          // Menampilkan kualitas udara
                          $('#qualityAQ1').text(predik.aqaq);
                          $('#qualityCEMS1').text(predik.cemsaq);

                          // Fungsi untuk mengubah warna latar belakang card berdasarkan status kualitas
                          function setCardBackgroundColor(cardId, quality) {
                            const cardElement = $(cardId);
                            console.log(`Setting background for ${cardId} with quality: ${quality}`);

                            cardElement.css('background-color', '');

                            if (quality === 'Normal') {
                              cardElement.css('background-color', '#9ade7b');
                            } else if (quality === 'Sedang') {
                              cardElement.css('background-color', '#fcde4d');
                            } else if (quality === 'Buruk') {
                              cardElement.css('background-color', '#eda8aa');
                            } else {
                              console.log('No matching quality found, defaulting to no color.');
                              cardElement.css('background-color', '');
                            }
                          }

                          // Atur warna latar belakang card berdasarkan kualitas AQI dan CEMS
                          setCardBackgroundColor('#aqiCard', predik.aqaq); // Mengatur warna berdasarkan kualitas AQI
                          setCardBackgroundColor('#cemsCard', predik.cemsaq); // Mengatur warna berdasarkan kualitas CEMS
